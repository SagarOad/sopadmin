...
interface Column {
  id: 'SNo' | 'Name' | 'quantity' | 'description' | 'action';
  label: string;
  minWidth?: number;
  align?: 'left';
...
interface Data {
  ...
  description: string;
  provinceName: string;
  consumedQauntity: string;
  inventoryName: string;
  InventoryId: number;
}

const ConsumedInventoryList = () => {
  const {
    state: { user },
  } = useAuth();
  ...
  const fetchList = async () => {
    try {
      const response = await axios.get(`/ConsumedItemList`, {
        params: {
          id: user?.user_id,
        },
        headers: {
      ...
      setPhaseList(response?.data?.consumedItems);
  ...
  const fetchProvince = async () => {
    try {
      const response = await axios.get(`/consumableItemList`, {
        params: {
          id: user?.user_id,
        },
        headers: {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    const AddDistrict = async () => {
      if (formRef.current) {
        ...
        try {
          const response = await axios.post(`/updateQuantity`, formData, {
            headers: {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Inventory / Consumed Inventory List" />
                      ...
                      phaseList
                        ...
                        .map((phase: Data, index: number) => (
                          <TableRow className="bg-light" key={phase?.id}>
                            ...
                            <TableCell>{phase?.consumedQauntity}</TableCell>
                          </TableRow>
                        ))}
                  </TableBody>
                </Table>
      ...
      {showFormModal ? (
        <div className="fixed inset-0 z-9999 flex items-center justify-center overflow-y-auto overflow-x-hidden outline-none focus:outline-none">
          ...
          <h4 className='text-center text-2xl'>Add Consumed Items</h4>
...
export default ConsumedInventoryList;
